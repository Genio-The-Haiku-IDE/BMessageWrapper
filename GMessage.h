...
public:
	static T	Get(GMessage* msg, const char* key){
				if constexpr (std::is_pointer<T>::value) {
					T error;
					return You_need_to_cast_to_Void(error);
				} else {
					T* write_specific_converter;
					return Unsupported_MessageValue_Get_for_GMessage(write_specific_converter);
				}
	}
	static void	Set(GMessage* msg, const char* key, T value){
				if constexpr (std::is_pointer<T>::value) {
					T error;
					You_need_to_cast_the_returned_pointer(error);
				} else {
					T* write_specific_converter;
					Unsupported_MessageValue_Set_GMessage(write_specific_converter);
				}
	}
...
#define MESSAGE_VALUE_MIX(NAME, TYPE, typeCODE, DEFAULT) \
template<> \
class MessageValue<TYPE> { \
public: \
	static TYPE		Get(GMessage* msg, const char* key) { \
...
MESSAGE_VALUE_MIX(Pointer, void*, B_POINTER_TYPE, nullptr);

MESSAGE_VALUE(Color, rgb_color, B_RGB_COLOR_TYPE, rgb_color());

MESSAGE_VALUE_MIX(Messenger, BMessenger, B_MESSENGER_TYPE, BMessenger());

MESSAGE_VALUE_REF(Ref, entry_ref, B_REF_TYPE, entry_ref());
MESSAGE_VALUE_REF(NodeRef, node_ref, B_NODE_REF_TYPE, node_ref());
